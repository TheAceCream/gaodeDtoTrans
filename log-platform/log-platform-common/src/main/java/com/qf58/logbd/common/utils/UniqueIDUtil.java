	...
	private static Callable<Integer> uidGenerateTask = new Callable<Integer>() {
		...
		public Integer call() {
			...
			if (UniqueIDUtil.uniqueIdQueue.size() < UniqueIDUtil.sizeDivide) {
				...
				lastSize = UniqueIDUtil.uniqueIdQueue.size();
		...
		private void reDetermineIndicator() {
			...
			if (UniqueIDUtil.scaleQueue.size() > 3) {
				totalScale -= UniqueIDUtil.scaleQueue.poll();
			}

			if (UniqueIDUtil.scale != UniqueIDUtil.totalScale / UniqueIDUtil.scaleQueue.size()) {
				scale = UniqueIDUtil.totalScale / UniqueIDUtil.scaleQueue.size();
				sizeDivide = UniqueIDUtil.DEFAULT_SIZE_DIVIDE << UniqueIDUtil.scale;
				sizeCeiling = (UniqueIDUtil.DEFAULT_SIZE_CEILING << UniqueIDUtil.scale) + 5;
